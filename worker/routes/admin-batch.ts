...
import { runKvCursorBatch } from '../lib/batch-runner';
...
/**
 * Admin Batch Route Handlers
 *
 * Safety & Tool Configuration Specs:
 * 1. POST /api/admin/backfill-all-batch:
 *    - Cursor Key: BACKFILL_ALL_CURSOR (_backfill_all_cursor)
 *    - Default Batch Size: 2 channels
 *    - Worst-case touched per call: 2 channels, up to 40 API pages (2000 videos) each
 *    - Subrequest Budget Awareness: Kept to 2 channels to stay well within Cloudflare Worker 50 subrequests limit.
 *
 * 2. POST /api/admin/cleanup-dead-videos-batch:
 *    - Cursor Key: CLEANUP_DEAD_VIDEOS_CURSOR (_cleanup_dead_videos_cursor)
 *    - Default Batch Size: up to 15 channels or 2000 total videos
 *    - Worst-case touched per call: 15 channels / 2000 videos chunked by 50 in YouTube API calls (~40 subrequests max)
 *    - Subrequest Budget Awareness: 50-video chunking maintains safe subrequest counts under 50 limit.
 *
...
export async function handleAdminBatchRoutes(
  ...
  if (url.pathname === '/api/admin/cleanup-dead-videos-batch' && request.method === 'POST') {
    ...
    try {
      const result = await runKvCursorBatch<any, any, any>({
        env,
        cursorKey: CLEANUP_DEAD_VIDEOS_CURSOR,
        lockBy: 'manual_admin_cleanup',
        reset,
        dryRun,
        manageLock: true,
        totalItems: (channelsSeed as any[]).length,
        batchSize: 15,
        loadWorkItems: async (cursorBefore) => {
          ...
          return {
            items: [{ batchChannels, loadFailedChannels }],
            advanceCount: channelsIncluded,
          };
        },
        processWork: async ([{ batchChannels, loadFailedChannels }], cursorBefore, context) => {
          const failedChannels: any[] = [...loadFailedChannels];
          ...
          for (const { channel, videos } of batchChannels) {
            ...
            try {
              ...
              if (deadRemoved > 0 && env.CHANNELS_ARCHIVE && !context.dryRun) {
          ...
          if (env.CHANNELS_ARCHIVE && totalDeadVideosRemoved > 0 && fullMergedList.length > 0 && !context.dryRun) {
            try {
      ...
      if ('isLocked' in result && result.isLocked) {
        return new Response(
          JSON.stringify({
            error: `Maintenance is currently in progress (locked until ${result.lock?.until}). Please wait for the current batch to complete.`,
            lock: result.lock,
          }),
          { status: 409, headers: corsHeaders }
        );
  ...
  if (url.pathname === '/api/admin/scan-cleanup-batch' && request.method === 'POST') {
    ...
    try {
      const result = await runKvCursorBatch<any, any, any>({
        env,
        cursorKey: SCAN_CLEANUP_CURSOR,
        lockBy: 'manual_admin_scan_cleanup',
        reset,
        dryRun,
        manageLock: true,
        totalItems: (channelsSeed as any[]).length,
        batchSize: 1,
        loadWorkItems: (cursorBefore) => {
          const seed: any = (channelsSeed as any[])[cursorBefore];
          return { items: [seed], advanceCount: 1 };
        },
        processWork: async ([seed], cursorBefore, context) => {
          const sourceId = seed.sourceId;
          ...
          if (reset && env.CHANNELS_ARCHIVE && !context.dryRun) {
            try {
          ...
          if (channelVideos.length === 0 || failedChannels.length > 0) {
            const isChannelComplete = failedChannels.length === 0;
            return {
              processed: [
                {
                  sourceId,
              ...
              failed: failedChannels,
              advanceCountOverride: isChannelComplete ? 1 : 0,
              deleteOffsetKey: isChannelComplete ? scanCleanupOffsetKey(sourceId) : undefined,
              extraFields: {
                sourceId,
                title,
          ...
          return {
            ...
            failed: failedChannels,
            advanceCountOverride: isChannelComplete ? 1 : 0,
            deleteOffsetKey: isChannelComplete ? offsetKey : undefined,
            saveOffsetKey: !isChannelComplete ? { key: offsetKey, offset: newOffset } : undefined,
            extraFields: {
              sourceId,
              title,
